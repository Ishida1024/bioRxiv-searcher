...
それ以外の URL、改行、制御文字、過度に長い値、一般的な DOI 形式（`10.<registrant>/<suffix>`）に該当しない値は入力エラーとする。`10.1101/` は従来の bioRxiv DOI に多い形式だが、現在の検索結果には別の DOI プレフィックスも現れるため、プレフィックスを固定しない。正規化後の DOI は URL パスへ埋め込む前に URL エンコードする。ただし bioRxiv API の DOI パス区切りを維持するため、`/` はパス区切りとして扱う。
